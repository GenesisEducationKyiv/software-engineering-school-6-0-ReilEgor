...
import (
	...
	trackingMocks "github.com/GenesisEducationKyiv/software-engineering-school-6-0-ReilEgor/services/tracking/internal/mocks"
)
...
func TestCachedGitHubClient_RepoExists(t *testing.T) {
	...
	tests := []struct {
		name            string
		setupMock       func(mClient *trackingMocks.GitHubClient, mCache *sharedMocks.Cache)
		wantResult      bool
		wantErr         bool
		wantErrIs       error
	...
	}{
		{
			...
			setupMock: func(_ *trackingMocks.GitHubClient, mCache *sharedMocks.Cache) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			mClient := trackingMocks.NewGitHubClient(t)
			mCache := sharedMocks.NewCache(t)
