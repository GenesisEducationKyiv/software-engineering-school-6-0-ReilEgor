...
func (c *SubscriptionActivatedConsumer) consume(ctx context.Context) error {
	...
	if _, err = ch.QueueDeclare(sharedRabbitmq.QueueSubscriptionActivated, true, false, false, false, nil); err != nil {
		return fmt.Errorf("declare queue: %w", err)
	}

	msgs, err := ch.Consume(sharedRabbitmq.QueueSubscriptionActivated, "", false, false, false, false, nil)
	if err != nil {
...
func (c *SubscriptionActivatedConsumer) handle(ctx context.Context, d amqp.Delivery) {
	var event sharedModel.SubscriptionActivatedEvent
